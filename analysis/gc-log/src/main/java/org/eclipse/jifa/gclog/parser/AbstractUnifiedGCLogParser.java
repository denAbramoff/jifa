...
public abstract class AbstractUnifiedGCLogParser extends AbstractGCLogParser
{
    ...
    private static class JDK11LogLine
    {
        ...
        {
            if (timestamp == Constant.UNKNOWN_LONG && timestampString == null
                    && getUptime() == Constant.UNKNOWN_DOUBLE)
            { // need at least one
                return false;
            }
            if (getLoglevel() != null && !"info".equals(getLoglevel()))
            { // parse info level only now
                return false;
            }
            // need gc tag
            return getTags() == null || getTags().contains("gc");
        }
    }

    /**
     * [2021-05-06T11:25:16.508+0800][2021-05-06T03:25:16.508+0000][0
     * .202s][1620271516508ms][202ms][1489353078113131ns][201765461ns][B-K0S4ML7L-0237.local][45473][14083][info][gc,
     * start     ] GC(0) Pause Young (Normal) (G1 Evacuation Pause)
     * A line in log may have multiple optimal decorations and some of them are redundant.
     * timestamp: [2021-05-06T11:25:16.508+0800][2021-05-06T03:25:16
     * .508+0000][1620271516508ms][1489353078113131ns][201765461ns]
     * uptime: [0.202s][202ms]
     * logLevel: info
     * tags: gc,start
     ...
     * see https://docs.oracle.com/javase/9/tools/java.htm#JSWOR-GUID-9569449C-525F-4474-972C-4C1F63D5C357
     * Decorations chapter
     */
    ...
    private static final Set<String> LOG_LEVEL_SET = Set.of("error", "warning", "info", "debug", "trace");
