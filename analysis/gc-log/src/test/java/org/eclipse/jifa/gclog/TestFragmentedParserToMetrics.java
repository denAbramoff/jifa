...
public class TestFragmentedParserToMetrics
{
    ...
    private List<Metric> parse(List<String> context, long startTime, long endTime)
    {
        ...
        Map<String, String> instanceId = new ImmutableMap.Builder<String, String>().put(INSTANCEIDKEY, INSTANCEIDVALUE)
                .build();
        try
        {
    ...
    public void testAnalyseSpeed()
    {
        ...
        files.forEach(file ->
        {
            ...
            Map<String, String> instanceId = new ImmutableMap.Builder<String, String>().put(INSTANCEIDKEY, file)
                    .build();
            ...
            System.out.printf(
                    "file %s analysed，%d lines in total，%d metrics produced，duration %fs，%f lines processed per "
                            + "minute，producing %f metrics\n",
                    file, gclog.size(), result.size(), spendTime, gclog.size() / spendTime * 60,
                    result.size() / spendTime * 60);
        });
    }
    ...
    public void testMetricLength()
    {
        ...
        metricSizeForEachGCLog.forEach((gclog, size) ->
                Assertions.assertEquals(
                        Integer.valueOf(parse(TestUtil.generateShuffledGCLog(gclog), 0, Long.MAX_VALUE).size()), size));
    }
        ...
        valueForEachMetric.forEach((metricName, labelMap) ->
                labelMap.forEach((label, value) ->
                {
                    List<Metric> actualMetrics = metrics.stream()
                            ...
                            .filter(metric -> metric.label().get("type").equals(label))
                            .filter(metric -> metric.timestamp() == 1669618629974L)
                            .filter(metric -> metric.label().get("gc_type").equals("CMS GC"))
                            .filter(metric -> metric.label().get(INSTANCEIDKEY).equals(INSTANCEIDVALUE))
                            .toList();
                    Assertions.assertEquals(actualMetrics.size(), 1);
                    Assertions.assertEquals(actualMetrics.getFirst().value(), value, DELTA);
                })
        );
    }
    ...
    public void testMetricContentSubphase()
    {
        ...
        {
            List<Metric> actualMetrics = metrics.stream()
                    .filter(metric -> metric.timestamp() == 1693292160355L)
                    .filter(metric -> metric.name().equals("GC_SUBPHASE_TIME"))
                    .filter(metric -> metric.label().get("gc_type").equals("CMS GC"))
                    .filter(metric -> metric.label().get(INSTANCEIDKEY).equals(INSTANCEIDVALUE))
                    .filter(metric -> metric.label().get("type").equals("CMS"))
                    .filter(metric -> metric.label().get("subphase").equals(subphase))
                    .toList();
            Assertions.assertEquals(actualMetrics.size(), 1);
            Assertions.assertEquals(actualMetrics.getFirst().value(), value, DELTA);
        });
