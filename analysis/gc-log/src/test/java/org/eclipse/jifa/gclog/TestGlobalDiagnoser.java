...
import org.eclipse.jifa.gclog.diagnoser.GlobalDiagnoser.MostSeriousProblemSummary;
...
import static org.eclipse.jifa.gclog.diagnoser.AbnormalType.OUT_OF_MEMORY;
import static org.eclipse.jifa.gclog.diagnoser.AbnormalType.SYSTEM_GC;
...
public class TestGlobalDiagnoser
{
    ...
    public void testDiagnoseBasic() throws Exception
    {
        String log =
                """
                        3.765: [Full GC (Metadata GC Threshold) 3.765: [CMS: 92159K->92159K(92160K), 0.1150376 secs] 99203K->99169K(101376K), [Metaspace: 3805K->3805K(1056768K)], 0.1150614 secs] [Times: user=0.11 sys=0.00, real=0.12 secs]
        ...
        Assertions.assertEquals(model.getGcEvents().size(), 7);
        GlobalAbnormalInfo diagnose = new GlobalDiagnoser(model, defaultConfig(model)).diagnose();

        Map<String, List<Double>> seriousProblems = diagnose.seriousProblems();
        Assertions.assertEquals(seriousProblems.size(), 3);
        Assertions.assertArrayEquals(
                seriousProblems.get(METASPACE_FULL_GC.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[] { 3765, 12765, 103765, 120765, 155765 }, DELTA);
        Assertions.assertArrayEquals(seriousProblems.get(SYSTEM_GC.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[] { 80765 }, DELTA);
        Assertions.assertArrayEquals(
                seriousProblems.get(HEAP_MEMORY_FULL_GC.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[] { 95765 }, DELTA);

        MostSeriousProblemSummary mostSeriousProblem = diagnose.mostSeriousProblem();
        Assertions.assertEquals(mostSeriousProblem.problem(), new I18nStringView("jifa.gclog.diagnose.abnormal"
                + ".metaspaceFullGC"));
        Assertions.assertTrue(
                mostSeriousProblem.suggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion"
                        + ".checkMetaspace")));
        Assertions.assertTrue(
                mostSeriousProblem.suggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion"
                        + ".enlargeMetaspace")));
        ...
        List<TimeRange> sites = mostSeriousProblem.sites();
        Assertions.assertEquals(sites.size(), 2);
        double[] actual = sitesToArray(sites);
        Assertions.assertArrayEquals(actual, new double[] { 0, 42765 + 115.0614, 73765, 155765 + 115.0614 }, DELTA);
        ...
        seriousProblems = diagnose.seriousProblems();
        Assertions.assertEquals(seriousProblems.size(), 1);
        Assertions.assertEquals(seriousProblems.get(METASPACE_FULL_GC.getName()).size(), 2);
    }
    ...
    public void testNoProblem() throws Exception
    {
        String log =
                """
                        2020-12-27T00:29:54.757+0800: 1062298.547: [GC (Allocation Failure) 2020-12-27T00:29:54.757+0800: 1062298.547: [ParNew: 1826919K->78900K(1922432K), 0.0572643 secs] 3445819K->1697799K(4019584K), 0.0575802 secs] [Times: user=0.21 sys=0.00, real=0.05 secs]
        ...
        Assertions.assertEquals(diagnose.seriousProblems().size(), 0);
        Assertions.assertNull(diagnose.mostSeriousProblem());
    }
    ...
    public void testOrderSites() throws Exception
    {
        String log =
                """
                        4.204: [GC (Allocation Failure) 4.204: [ParNew: 7575411K->562327K(7689600K), 0.4413861 secs] 8149808K->1188439K(20272512K), 0.4422627 secs] [Times: user=1.33 sys=0.07, real=0.44 secs]
        ...
        List<TimeRange> sites = mostSeriousProblem.sites();
        Assertions.assertEquals(sites.size(), 3);
        double[] actual = sitesToArray(sites);
        Assertions.assertArrayEquals(actual,
                new double[] { 0, 54204 + 442.2627, 174204, 304204 + 442.2627, 374204, 444204 + 442.2627 }, DELTA);
    }
    ...
    public void testDiagnoseZGC() throws Exception
    {
        ...
        Map<String, List<Double>> seriousProblems = diagnose.seriousProblems();
        Assertions.assertEquals(seriousProblems.size(), 2);
        Assertions.assertArrayEquals(
                seriousProblems.get(ALLOCATION_STALL.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[] { 7777 - 0.204, 7888 - 0.391 }, DELTA);
        Assertions.assertArrayEquals(
                seriousProblems.get(OUT_OF_MEMORY.getName()).stream().mapToDouble(d -> d).toArray(),
                new double[] { 7889 }, DELTA);

        MostSeriousProblemSummary mostSeriousProblem = diagnose.mostSeriousProblem();
        Assertions.assertEquals(mostSeriousProblem.problem(),
                new I18nStringView("jifa.gclog.diagnose.abnormal.outOfMemory"));
        Assertions.assertTrue(
                mostSeriousProblem.suggestions().contains(new I18nStringView("jifa.gclog.diagnose.suggestion"
                        + ".checkMemoryLeak")));
        Assertions.assertTrue(
        ...
        List<TimeRange> sites = mostSeriousProblem.sites();
        Assertions.assertEquals(sites.size(), 1);
        double[] actual = sitesToArray(sites);
        Assertions.assertArrayEquals(actual, new double[] { 0, 7889 }, DELTA);
    }
