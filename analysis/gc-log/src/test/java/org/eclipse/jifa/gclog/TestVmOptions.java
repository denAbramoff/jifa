...
public class TestVmOptions
{
    ...
    public void testVmOptions()
    {
        ...
        Assertions.assertTrue(result.getOther()
                .contains(new VmOptions.VmOptionVo("-XX:ErrorFile=/home/admin/logs/hs_err_pid%p.log")));
        Map<String, Integer> optionIndex = new HashMap<>();
        for (int i = 0; i < result.getGcRelated().size(); i++)
        {
            optionIndex.put(result.getGcRelated().get(i).text(), i);
        }
        ...
        Assertions.assertTrue(
                optionIndex.get("-XX:ParallelGCThreads=8") < optionIndex.get("-XX:InitiatingHeapOccupancyPercent=50"));
        Assertions.assertTrue(
                optionIndex.get("-XX:InitiatingHeapOccupancyPercent=50") < optionIndex.get("-XX:-DisableExplicitGC"));
