...
import org.eclipse.jifa.jfr.exception.ProfileAnalysisException;
import org.eclipse.jifa.jfr.extractor.*;
import org.eclipse.jifa.jfr.extractor.Extractor;
import org.eclipse.jifa.jfr.extractor.JFRAnalysisContext;
...
{
    private static final Logger LOG = LoggerFactory.getLogger(JFRAnalyzerImpl.class);
    ...
    public JFRAnalysisContext getContext()
    {
        return context;
    }
    ...
    public AnalysisResult execute(AnalysisRequest request) throws ProfileAnalysisException
    {
        ...
        {
            if (e instanceof ProfileAnalysisException profileAnalysisException)
            {
                throw profileAnalysisException;
            }
    ...
    private AnalysisResult analyze(AnalysisRequest request) throws Exception
    {
        ...
        LOG.info("Analysis took {} milliseconds", r.getProcessingTimeMillis());
    ...
    private void processEvents(AnalysisRequest request, AnalysisResult r) throws Exception
    {
        ...
        {
            extractors.forEach(item -> doExtractorWork(events, item, r));
        }
        listener.worked(1);
    }
    ...
    private void transformEvents(AnalysisRequest request, IItemCollection collection) throws Exception
    {
        ...
        List<IItem> list = collection.stream().flatMap(IItemIterable::stream).toList();
    ...
    private void analyzeProblems(IItemCollection collection, AnalysisResult r)
    {
        ...
        for (IRule rule : RuleRegistry.getRules())
        {
            ...
            try
            {
                ...
                Severity severity = iResult.getSeverity();
                if (severity == Severity.WARNING)
                {
                    r.getProblems().add(new Problem(iResult.getSummary(), iResult.getSolution()));
                }
            }
            catch (Throwable t)
            {
    ...
    private List<Extractor> getExtractors(int dimensions)
    {
        ...
        extractorMap.forEach((item, extractor) ->
        {
