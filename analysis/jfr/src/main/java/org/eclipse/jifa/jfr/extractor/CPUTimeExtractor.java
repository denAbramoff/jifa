...
import org.eclipse.jifa.jfr.model.jfr.*;
import org.eclipse.jifa.jfr.model.*;
import org.eclipse.jifa.jfr.util.GCUtil;
import org.eclipse.jifa.jfr.util.StackTraceUtil;
import org.eclipse.jifa.jfr.util.TimeUtil;
...
{
    private static final Logger LOG = LoggerFactory.getLogger(CPUTimeExtractor.class);
    private static final List<String> INTERESTED = List.of(EventConstant.UNSIGNED_INT_FLAG,
            EventConstant.GARBAGE_COLLECTION, EventConstant.ACTIVE_SETTING, EventConstant.CPU_INFORMATION,
            EventConstant.ENV_VAR, EventConstant.THREAD_START, EventConstant.THREAD_CPU_LOAD,
    ...
    private List<TaskCPUTime> buildThreadCPUTime()
    {
        ...
        if (this.profiledByJFR)
        {
            ...
            if (gcTime > 0)
            {
                ...
                gcSamples.put(
                        StackTraceUtil.build(StackTraceUtil.newDummyStackTrace("", "JVM", "GC"), context.getSymbols()),
                        1L);
    ...
    private long buildGCCpuTime()
    {
        ...
        {
            return parallelGCThreads * parallelGCWallTime + concurrentGCThreads * concurrentGCWallTime
                    + serialGCWallTime;
        }
    }
