...
{
    private static final Logger LOG = LoggerFactory.getLogger(JStackParser.class);
    ...
    static final class PATTERNS
    {
        ...
        static Pattern patternOf(ParserImpl.Element element)
        {
            return switch (element)
            {
                ...
                case JNI_GLOBAL_REFS -> JNI_GLOBAL_REFS;
                case NON_JAVA_THREAD -> NO_JAVA_THREAD;
                default -> throw new ParserException("Should not reach here");
            };
        }
    }
    ...
    private static class ParserImpl
    {
        ...
        void parse(RawJavaThread rjt)
        {
            try
            {
                ...
                String line = contents.getFirst();
                Matcher m = PATTERNS.JAVA_THREAD.matcher(contents.getFirst());
                if (!m.matches())
                {
