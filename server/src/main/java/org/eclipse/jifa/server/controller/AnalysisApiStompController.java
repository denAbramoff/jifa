...
{
    private static final Logger LOG = LoggerFactory.getLogger(AnalysisApiStompController.class);
    ...
    handleRequest(@Header(name = StompHeaders.CONTENT_TYPE, required = false, defaultValue = Constant.APPLICATION_JSON)
    String contentType,
            @Header(name = Constant.STOMP_ANALYSIS_API_REQUEST_ID_KEY, required = false, defaultValue = "")
            String requestId,
    ...
    handleRequestException(Throwable throwable,
            @Header(name = Constant.STOMP_ANALYSIS_API_REQUEST_ID_KEY, required = false, defaultValue = "")
            String requestId)
    {
        log(throwable);
